...
export const CrawlReport: FC = () => {
	...
	const formClasses = useMemo(
		() => objStr({ flex: true, "gap-3": true, "items-center": !!errors.sitename, "items-end": !errors.sitename }),
		[errors.sitename]
	);
	...
	const inputClasses = useMemo(
		() =>
			objStr({
				...
				"min-w-[300px]": true,
				"border-slate-950": !errors.sitename,
				"border-red-700": !!errors.sitename,
			}),
		[errors.sitename]
	);
		...
		<>
			<form onSubmit={handleSubmit(onSubmit)} className={formClasses}>
				<div>
					...
					{errors.sitename && <span className="text-red-700 text-xs">{errors.sitename.message}</span>}
				</div>
				<button
					type="submit"
