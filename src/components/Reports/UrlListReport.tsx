...
export const UrlListReport: FC = () => {
	...
	const validateUrl = (urlList: string) => {
	...
	const formClasses = useMemo(
		() => objStr({ flex: true, "gap-3": true, "items-center": !!errors.urls, "items-end": !errors.urls }),
		[errors.urls]
	);
	...
	const inputClasses = useMemo(
		() =>
			objStr({
				...
				"min-w-[300px]": true,
				"border-slate-950": !errors.urls,
				"border-red-700": !!errors.urls,
			}),
		[errors.urls]
	);
		...
		<>
			<form onSubmit={handleSubmit(onSubmit)} className={formClasses}>
				<div>
					...
					{errors.urls && <span className="text-red-700 text-xs">{errors.urls.message}</span>}
				</div>
				<button
					type="submit"
